...
const CreditsPage = () => {
  const { credits, userPickups } = useAppContext();
  const level = Math.floor(credits / 500) + 1;

  const pickups = Array.isArray(userPickups) ? userPickups.filter(p => p.status === 'Completed') : [];
          ...
          {pickups.length > 0 ? (
            pickups.slice(0, 3).map((pickup, index) => (
              <div key={pickup.id} style={{
                padding: '1rem',
                borderBottom: index < pickups.slice(0, 3).length - 1 ? '1px solid #f0f0f0' : 'none',
                display: 'flex',
