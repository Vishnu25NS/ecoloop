    ...
    export const AppProvider = ({ children }) => {
      ...
      const login = async (email, password) => {
        try {
          ...
          const userDocRef = doc(db, 'artifacts', process.env.REACT_APP_FIREBASE_PROJECT_ID, 'users', user.uid, 'profile', 'data');
          const userDocSnap = await getDoc(userDocRef);
          if (userDocSnap.exists()) {
            const userData = userDocSnap.data();
            setUser({ ...user, ...userData, roles: userData.roles || ['user'] });
      ...
      const addCredits = async (amount) => {
        if (!userId || !db) return;
        const userProfileRef = doc(db, 'artifacts', process.env.REACT_APP_FIREBASE_PROJECT_ID, 'users', userId, 'profile', 'data');
      ...
      const addClassificationToHistory = async (item) => {
        if (!userId || !db) return;
        const historyRef = collection(db, 'artifacts', process.env.REACT_APP_FIREBASE_PROJECT_ID, 'users', userId, 'classificationHistory');
      ...
      const addPickup = async (pickup) => {
        if (!userId || !db) return;
        const userPickupsRef = collection(db, 'artifacts', process.env.REACT_APP_FIREBASE_PROJECT_ID, 'users', userId, 'pickups');
        ...
        const availablePickupsRef = collection(db, 'artifacts', process.env.REACT_APP_FIREBASE_PROJECT_ID, 'public', 'data', 'availablePickups');
      ...
      const scheduleNewPickup = async (pickup) => {
        if (!userId || !db) return;
        const availablePickupsRef = collection(db, 'artifacts', process.env.REACT_APP_FIREBASE_PROJECT_ID, 'public', 'data', 'availablePickups');
        const assignedPickupsRef = collection(db, 'artifacts', process.env.REACT_APP_FIREBASE_PROJECT_ID, 'public', 'data', 'assignedPickups');
        ...
        const userPickupsRef = collection(db, 'artifacts', process.env.REACT_APP_FIREBASE_PROJECT_ID, 'users', pickup.userId, 'pickups');
        await addDoc(userPickupsRef, {
          ...pickup,
          collectorId: userId,
          collectorName: user.name,
      ...
      const updatePickupStatus = async (pickupId, newStatus, notes = '') => {
        if (!db || !userId) return;
        const pickupRef = doc(db, 'artifacts', process.env.REACT_APP_FIREBASE_PROJECT_ID, 'public', 'data', 'assignedPickups', pickupId);
